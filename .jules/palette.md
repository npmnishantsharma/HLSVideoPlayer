...
## 2024-05-24 - Icon-only Components Title Attribute
**Learning:** In addition to `aria-label`s for screen readers, icon-only components (such as `<md-icon-button>`) need to include a `title` attribute to provide visual hover tooltips for mouse users.
**Action:** Always ensure that icon-only components have a `title` attribute to provide a visual tooltip alongside the `aria-label` for screen reader users.
